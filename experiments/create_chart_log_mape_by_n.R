...
#ns        <- c(50, 100, 500, 1000, 2500)

# --- Collect MAPE + CI across n & rho ---
grid <- tidyr::expand_grid(n = ns, rho = rhos)

get_mape_row <- function(n, rho) {
  b <- load_bundle(n = n, rho = rho, link_type = link_type, p_base = p_base)
  
  d_grf  <- mape_from_bundle_detail(b, forest_type = "grf")   # trial-level t-interval
...
df_mape_n <- purrr::pmap_dfr(grid, get_mape_row) |>
  mutate(
    method = factor(method, levels = c("RRCF", "GRF")),  # blue RRCF, green GRF
    n      = factor(n, levels = ns)
  )

# --- Plot (5 panels), percent y, dodged/thinner CI bars ---
pd   <- position_dodge(width = 0.02)
xmin <- min(df_mape_n$rho) - 0.02
xmax <- max(df_mape_n$rho) + 0.02

p_mape_by_n <- ggplot(df_mape_n, aes(x = rho, y = value, color = method, group = method)) +
  ...
  labs(
    ...
    subtitle = "Error bars: 95% CIs (trial-level t-interval)"
  ) +
...
outfile   <- file.path("charts", paste0("log_mape_by_n_", timestamp, ".pdf"))
ggsave(outfile, plot = p_mape_by_n, width = 18, height = 5.5, units = "in")
