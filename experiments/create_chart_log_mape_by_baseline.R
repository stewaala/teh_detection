# Assumes:
#   load_bundle()
#   mape_from_bundle_detail()  # returns estimate + CI
...
# --- Collect MAPE + CI across p_base & rho ---
grid <- tidyr::expand_grid(p_base = p_bases, rho = rhos)

get_mape_row <- function(p_base, rho) {
  pb <- if (is.na(p_base)) NULL else p_base
  b  <- load_bundle(n = n, rho = rho, link_type = link_type, p_base = pb)
  
  d_grf  <- mape_from_bundle_detail(b, forest_type = "grf")   # trial-level t-interval (default)
...
df_mape_pb <- purrr::pmap_dfr(grid, get_mape_row) |>
  mutate(
...
xmin <- min(df_mape_pb$rho) - 0.02
xmax <- max(df_mape_pb$rho) + 0.02

p_mape_by_pbase <- ggplot(df_mape_pb, aes(x = rho, y = value, color = method, group = method)) +
  geom_line(linewidth = 1.1, position = pd) +
  geom_point(size = 2.6, position = pd) +
  geom_errorbar(
    aes(ymin = ci_lower, ymax = ci_upper),
    position = pd,
    width = 0.018,     # shorter caps
  ...
  labs(
    ...
    subtitle = "Panels: default, p_base=0.05, p_base=0.01 (95% CIs; trial-level t-interval)"
  ) +
  facet_wrap(~ p_base_label, nrow = 1, scales = "free_y") +
  theme_minimal(base_size = 12) +
  theme(panel.grid.minor = element_blank())

print(p_mape_by_pbase)

# --- Save PDF to ./charts with timestamp yyyymmddhhmm ---
if (!dir.exists("charts")) dir.create("charts", recursive = TRUE)
timestamp <- format(Sys.time(), "%Y%m%d%H%M")
outfile   <- file.path("charts", paste0("log_mape_by_baseline_", timestamp, ".pdf"))
ggsave(outfile, plot = p_mape_by_pbase, width = 13, height = 5.5, units = "in")
